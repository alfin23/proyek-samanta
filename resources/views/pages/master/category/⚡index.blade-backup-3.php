...
{{-- x-data="{ open: false }" = mendefinisikan variable lokal browser bernama open --}}
{{-- @ adalah singkatan dari x-on: (perintah Alpine untuk mendengarkan sesuatu). --}}
{{-- category-saved adalah nama event yang dikirim dari server lewat $this->dispatch('category-saved'). --}}
{{-- .window artinya: "Dengarkan sinyal ini di seluruh lingkup halaman browser." --}}
{{-- ="open = false" artinya: "Jika sinyalnya terdengar, langsung tutup modalnya!" --}}

<div x-data="{ open: false }" @category-saved.window="open = false" class="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
    <div
        ...
        {{-- @click="open = true" akan langsung membuka modal di browser tanpa delay sekecil apa pun. Kemudian $wire.openModal() berjalan di latar belakang untuk memberi tahu fungsi openModal() di PHP agar melakukan resetErrorBag() dan membersihkan input --}}
        <button @click="open = true; $wire.openModal()" class="btn btn-primary shadow-sm">
    ...
    {{-- :class={ 'modal-open': open } = Tambahkan class modal-open jika variable open bernilai true --}}
    {{-- x-show="open" = tampilkan elemen ini hanya jika open bernilai true --}}
